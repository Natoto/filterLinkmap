...
- (void)viewDidLoad {
    ...
    NSString * linkMapFileURL = [NSUserDefaults.standardUserDefaults stringForKey:@"linkMapFileURLHistory"];
    if (linkMapFileURL) {
        _linkMapFileURL = [NSURL fileURLWithPath:linkMapFileURL];
        _filePathField.stringValue = linkMapFileURL;
    }
    NSString * historysearch = [NSUserDefaults.standardUserDefaults stringForKey:@"searchFieldHistory"];
    if (historysearch) {
        self.searchField.stringValue = historysearch;
    }
}
...
- (IBAction)analyze:(id)sender {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (1 == groupButtonState) {
            [self buildCombinationResultWithSymbols:sortedSymbols searchKey:searchKey];
        } else {
...
- (void)buildCombinationResultWithSymbols:(NSArray *)symbols searchKey:(NSString*)searchKey{
    self.result = [@"库大小\t库名称\r\n\r\n" mutableCopy];
    NSUInteger totalSize = 0;
...
- (void)appendResultWithSymbol:(SymbolModel *)model {
    NSString *size = nil;
    NSString *size2 = nil;
    size = [NSString stringWithFormat:@"%ld", model.size];
    size2 = [NSString stringWithFormat:@"%ld", model.size];
    ...
    [_result appendFormat:@"%@\t\t%@\t\t%@\r\n",size2,size, [[model.file componentsSeparatedByString:@"/"] lastObject]];
}
